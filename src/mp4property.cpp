                            ...
                            bool dumpImplicits, uint32_t index)
{
    ...
    if( size == 0 ) {
        log.dump(indent, MP4_LOG_VERBOSE1, "%s = <%u bytes>",
                 m_name, size );
        return;
    }
    ...
    if( size <= 16 ) {
        ...
        log.dump(indent, MP4_LOG_ERROR, "%s = <%u bytes>%s",
    ...
    if( showall ||
        size < 256 || log.verbosity >= MP4_LOG_VERBOSE2 )
    {
        adjsize = size;
        supressed = false;
    }
    ...
    log.dump(indent, MP4_LOG_VERBOSE1, "%s = <%u bytes>",
             m_name, size );
    log.hexDump(indent, MP4_LOG_VERBOSE1, value, adjsize, "%s",
                m_name);
...
{
    log.dump(indent, MP4_LOG_VERBOSE1, "%s = %s (0x%02x)",
             m_name,
